...
USE_TEXTLINE_ORIENTATION = os.environ.get("BARCODE_TEXTLINE_ORIENTATION", "1") not in ("0", "false", "False")
...
KNOWN_CODE_ACCEPT_CONFIDENCE = 0.75
NG_SEARCH_BUDGET_SECONDS = 4.5


def _load_barcode_codes():
    """Load the dataset-derived code list without making it a hard whitelist."""
    runtime_dir = (
        Path(sys.executable).resolve().parent
        if getattr(sys, "frozen", False)
        else Path(__file__).resolve().parent
    )
    configured = os.environ.get("BARCODE_CODES_FILE", "").strip()
    paths = []
    if configured:
        paths.append(Path(configured))
    paths.extend((runtime_dir / "barcode_codes.json", runtime_dir.parent / "barcode_codes.json"))
    ...
    for path in paths:
        try:
            ...
            valid = {
                str(code).upper() for code in codes
                if BARCODE_REGEX.fullmatch(str(code).upper())
            }
            if valid:
                return valid, path
...
KNOWN_BARCODE_CODES, BARCODE_CODES_PATH = _load_barcode_codes()
...
BARCODE_OCR_ALIASES = {
    ...
    "S5T": "LSS",
    "SS7": "LSS",
    # Confirmed FT0B glyph errors from the labelled Barcode_OK dataset.
    "FT00": "FT0B",
    "FT0P": "FT0B",
    "FTUB": "FT0B",
    "FT0R": "FT0B",
...
class OCRModel:
    ...
    def extract_barcodes(self, result, stage_label, score_threshold=0.15, deskew_angle=0.0):
        candidates = []
        ...
        for i, (text, score) in enumerate(zip(rec_texts, rec_scores)):
            ...
            for code in parts:
                ...
                if not BARCODE_REGEX.fullmatch(code) and code.endswith("L3L"):
                    code = "L3L"

                # Restore one clipped trailing character only when the loaded
                # production list has exactly one possible continuation. This
                # is deterministic (FT0 -> FT0B), not general fuzzy matching.
                if len(code) == 3 and code not in KNOWN_BARCODE_CODES:
                    completions = [known for known in KNOWN_BARCODE_CODES if known.startswith(code)]
                ...
                candidates.append({"label": code, "confidence": float(score), "position": position, "stage": stage_label, "known": code in KNOWN_BARCODE_CODES, "deskew_angle": deskew_angle})
    ...
    def _order_barcode_pair(self, first, second, image_shape):
        """Order two barcodes found in the SAME OCR pass: side-by-side ->
        rightmost first; stacked -> topmost first."""
        ax, ay = self._center_in_normal_view(first, image_shape)
        bx, by = self._center_in_normal_view(second, image_shape)
        if abs(ax - bx) >= abs(ay - by):
            return (first, second) if ax >= bx else (second, first)
        return (first, second) if ay <= by else (second, first)

    def _center_in_normal_view(self, candidate, proc_shape):
        """Map a candidate center back to the un-rotated, un-deskewed
        preprocessed frame so positions from different stages/rotations are
        directly comparable.

        `proc_shape` must be the shape of `preprocessed_bgr` (the deskewed
        frame that Stage1/Barcode90/180/270 all derive from) — that is the
        coordinate space `candidate['position']` was measured in.
        """
        x1, y1, x2, y2 = candidate['position']
        x = (x1 + x2) / 2.0
        y = (y1 + y2) / 2.0
        height, width = proc_shape[:2]

        # Undo the fixed 90/180/270 barcode-orientation rotation first,
        # landing back in the deskewed (but not yet de-rotated) frame.
        if candidate['stage'] == "Barcode90":
            x, y = y, height - 1 - x
        elif candidate['stage'] == "Barcode180":
            x, y = width - 1 - x, height - 1 - y
        elif candidate['stage'] == "Barcode270":
            x, y = width - 1 - y, x

        # Undo the weighted-deskew rotation. Without this, a deskew estimate
        # near +-90 deg (seen on some frames with strong vertical edges)
        # silently swaps left/right with top/bottom, scrambling the
        # two-barcode left/right ordering below.
        angle = candidate.get('deskew_angle', 0.0)
        if angle:
    ...
    def _run_ocr_on(self, image_bgr, stage_label, deskew_angle=0.0):
        candidates = []
        raw_texts = []
        try:
            with Profiler(f"PaddleOCR ({stage_label})"):
                result = self.ocr.predict(image_bgr)
            candidates = self.extract_barcodes(result, stage_label, deskew_angle=deskew_angle)
    ...
    # OCR_System is deployed. BARCODE_RESULT_BASE overrides it if needed.
    RESULT_BASE = os.environ.get(
        "BARCODE_RESULT_BASE",
        os.path.join(os.path.splitdrive(os.path.abspath(__file__))[0] + os.sep, "result", "barcode_result"),
    )
    ...
    def detectJSON(self):
        ...
        # Hard wall-clock cap for the fallback chain (rotations + distant
        # zoom). Stage 1 always runs to completion — it is the accurate,
        # cheap path — this only stops the expensive NG-search fallbacks
        # once their time is spent, so real NG detection can't run away
        # past the director's 5s limit the way it used to.
        deadline = total_timer + NG_SEARCH_BUDGET_SECONDS
        all_candidates = []
        ...
        found1, raw1 = self._run_ocr_on(preprocessed_bgr, "Stage1", deskew_angle=deskew_angle)
        ...
        def has_reliable(candidates):
            # A one-pass unknown 3-4 letter word can be body/label text (OSM,
            # OSW, TRM...). Only known production codes may short-circuit.
            return any(
                candidate['known']
                and candidate['confidence'] >= KNOWN_CODE_ACCEPT_CONFIDENCE
                for candidate in candidates
            )

        def has_known_reliable(candidates):
            return any(
                candidate['known']
                and candidate['confidence'] >= (
                    0.70 if candidate['stage'].startswith('BarcodeBand/')
                    else KNOWN_CODE_ACCEPT_CONFIDENCE
                )
                for candidate in candidates
            )

        stage1_is_reliable = has_reliable(found1)
        if stage1_is_reliable:
            ...
            for stage_name, rotation in barcode_orientations:
                if time.monotonic() >= deadline:
                    log("Rotation fallback: time budget reached, stopping early", "WARN")
                    break
                with Profiler(f"  Rotate {stage_name}"):
                    rotated_barcode = cv2.rotate(preprocessed_bgr, rotation)
                found_rotated, raw_rotated = self._run_ocr_on(rotated_barcode, stage_name, deskew_angle=deskew_angle)
        ...
        if not all_candidates and time.monotonic() < deadline:
            ...
            if distant_crop is not None:
                ...
                for stage_name, rotation in distant_orientations:
                    if time.monotonic() >= deadline:
                        log("Distant zoom fallback: time budget reached, stopping early", "WARN")
                        break
        ...
        if all_candidates:
            ...
            # ── Two barcodes in one frame ──
            # Merge in a second code only when it was found in the SAME
            # OCR pass as `best` (so positions are directly comparable,
            # i.e. it is truly a second label in the same photo, not a
            # coincidence from a different rotation/crop attempt) AND
            # both codes are independently trustworthy on their own
            # (known code list + high confidence, or very high raw
            # confidence). This guards against ever gluing an unrelated
            # recognized word onto a real barcode.
            best_is_trustworthy = best in known_candidates or best['confidence'] >= DIRECT_ACCEPT_CONFIDENCE
            if best_is_trustworthy:
                companions = [
                    c for c in all_candidates
                    if c['stage'] == best['stage'] and c['label'] != best['label']
                    and (has_known_reliable([c]) or c['confidence'] >= DIRECT_ACCEPT_CONFIDENCE)
                ]
                if companions:
                    second = max(companions, key=lambda c: c['confidence'])
                    first_c, second_c = self._order_barcode_pair(best, second, preprocessed_bgr.shape)
                    final_label = f"{first_c['label']}_{second_c['label']}"
                    final_confidence = min(best['confidence'], second['confidence'])
                    log(f"Two barcodes detected: {Colors.BRIGHT_GREEN}{final_label}{Colors.RESET}", "SUCCESS")

            log(f"Best Barcode: {Colors.BRIGHT_GREEN}{final_label}{Colors.RESET} | {final_confidence:.3f} | {best['stage']}", "SUCCESS")
            results_list = [{"label": final_label, "confidence": final_confidence,
                             "stage": best['stage'], "known": best.get('known', False)}]
            self._save_result("OK", final_label, final_confidence, best['stage'])
        else:
