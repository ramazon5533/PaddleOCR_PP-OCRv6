...
class OCRModel:
    def __init__(self, effective_cpu_count=None):
        ...
        # Default assumes the Barcode server is normally running at the
        # same time on this same PC (that's the real factory setup,
        # whether started via Start_All_PP-OCRv6_Servers or launched
        # standalone / by the client app). Requesting 90% here AND 90% in
        # Barcode independently would oversubscribe the CPU by ~180%,
        # causing wildly inconsistent per-image latency (confirmed in the
        # field: OCR calls that normally take ~2s spiking to 6-10s once
        # both servers were actively processing at once). Set
        # OCR_CPU_THREADS explicitly for a higher share when only this
        # server runs alone (e.g. isolated testing) -
        # Start_All_PP-OCRv6_Servers already does this automatically for
        # a coordinated dual-server launch. VIN gets a slightly larger
        # default share than Barcode since it has the tighter timing
        # requirement.
        base_count = effective_cpu_count or (os.cpu_count() or 1)
        cpu_threads = max(1, int(os.environ.get(
            "OCR_CPU_THREADS", str(max(1, round(base_count * 0.50)))
        )))
